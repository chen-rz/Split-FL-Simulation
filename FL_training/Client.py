...
class Client(Communicator):
	def __init__(self, index, ip_address, server_addr, server_port, datalen, model_name, split_layer):
		...
		# self.device = torch.device('cuda:' + str(index % 8) if torch.cuda.is_available() else 'cpu')
		...
		self.sock.bind(('127.0.0.1', 25000 + index))
		self.port = 25000 + index
	...
	def train(self, trainloader):
		...
		msg = ['MSG_TEST_NETWORK', self.port, network_speed]
		...
		if self.split_layer == (config.model_len -1): # No offloading training
			# TODO No tqdm for background-running simulation
			...
			for batch_idx, (inputs, targets) in enumerate(trainloader):
				...
				msg = ['MSG_LOCAL_ACTIVATIONS_CLIENT_TO_SERVER', outputs, targets]
				self.send_msg(self.sock, msg)
		...
		msg = ['MSG_TRAINING_TIME_PER_ITERATION', self.port, training_time_pr]
	...
	def upload(self):
		...
		# self.net.to("cuda:0")
		# msg = ['MSG_LOCAL_WEIGHTS_CLIENT_TO_SERVER', self.net.state_dict()]
		self.send_msg(self.sock, msg)
